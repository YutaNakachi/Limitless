...
| 日付 | 内容 |
| :--- | :--- |
| **05/28** | **特異な術式「赫」・「蒼」の基礎システム実装と演出基盤の構築**<br>・空間起爆（空振り時の自動展開）に対応した、衝突・失速時にその場に展開する術式「赫」を実装。<br>・「赫」の仕様に合わせ、サイズ拡大、敵個体ごとの無敵時間を管理した0.5秒間隔の持続多段ヒットシステムを構築。<br>・対象レイヤー（Enemy、Wall、Ground等）をインスペクターから一括指定できるよう `LayerMask` 判定を導入。<br>・これに伴い、`CollisionDetector` を拡張し、トリガー検知と物理衝突（Collision）のイベント分離に対応。<br>・発動時に一瞬で最大サイズまで広がり、範囲内の敵を中心へと滑らかに吸い寄せる引力の術式「蒼」を実装。<br>・ボールの速度（`linearVelocity.magnitude`）を常時監視し、失速時に煙エフェクトを自動消去するUniTask非同期制御を導入。<br>・演出強化の調査として、エフェクトの減速設定（Dampen）、ヒットストップ中の時間軸を無視したエフェクト駆動（Unscaled設定）、Point Lightによる周囲への照り返し表現の仕様を整理。 |
