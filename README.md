...
| 日付 | 内容 |
| :--- | :--- |
| **05/28** | **術式「赫」・「蒼」の実装と衝突・エフェクト制御の拡張**<br>・衝突や失速時に展開し、巨大化しながら0.5秒刻みで持続ダメージを与える斥力の術式「赫」を実装。<br>・発動時に一瞬で最大展開し、範囲内の敵を物理演算で中心へ滑らかに吸い寄せる引力の術式「蒼」を実装。<br>・トリガー判定と物理衝突（壁・床）を分離して検知・登録できるよう `CollisionDetector` を拡張。<br>・ボールの速度を常時監視してエフェクトを自動消去するUniTask制御や、時間停止中でも動く演出仕様を確立。 |
